# MIPS_CPU_Simulator
This project was done as coursework for a 2nd year Computer Architecutre Module. 
The aim was to build a CPU Simulator that would execute MIPS 1 big-endian binaries accurately. A testbench was also developed to test the simulator. 
...
## Simulator
The simulator is built using the command:
<pre><code>make simulator
</code></pre>

in the root of the respository. 
This should result in a binary called bin/mips_simulator. 

A binary called x.bin can be executed by the simulator using:

<pre><code>bin/mips_simulator x.bin
</code></pre>
...
## Testbench
The Testbench  takes a single command-line parameter, which is the path of the Simulator to be tested.
The Testbench can be run using the command:
<pre><code>./bin/mips_testbench [path of simulator executable]
</code></pre>
This will run the testbench bash script in the bin folder.

As output, the Testbench prints a CSV file to stdout, where each row of the file corresponds to 
exactly one execution of the Simulator under test. Each row has the following fields:

<pre><code>TestId , Instruction , Status , Author [, Message]
</code></pre>
...
All temporary and working files created during execution are created in a directory called test/temp. 
All output files of the testebnch are created in test/output. 

# Coursework Specifications

The original specification provided by the lecturer in charge of this coursework is provided [here](https://github.com/m8pple/arch2-2019-cw/blob/master/readme.md).
It is also as follows here:
