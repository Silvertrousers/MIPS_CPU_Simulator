...
int main(int argc, char *argv[] ){
  ...
  while(s.pc != 0){//go instruction by instruction until jump to 0, only works if i have a valid .bin file
    ...
    if((0x10000000 <= s.pc) && (s.pc < 0x11000000)){//checks if pc is looking at correct memory location
      ...
      s.instruction_look_up(inst);//to do: need to make sure the input to the
                                  //instructions inside this aare not just declared but are aassociated with the member values of inst.
                                  //maaybe checking should be done in this function in the case statement before everything is called
       //go to next instruction
    }
    else{
      std::exit(-12);//if pc goes too high, return memory error since trying to execute from wrong part of mem
    }
